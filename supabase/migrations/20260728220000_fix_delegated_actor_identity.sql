...
-- Fix delegated decision history display identity.
-- The immutable audit already stores actor_profile_id = auth.uid(). Prefer that
-- authenticated profile for the display name, then fall back to the employee row.
...
as $function$
  ...
  select
    ...
    audit.delegation_id,
    coalesce(
      nullif(trim(actor_profile.full_name), ''),
    ...
    coalesce(
      nullif(trim(primary_profile.full_name), ''),
  ...
  left join public.profiles primary_profile
    on primary_profile.id = primary_manager.user_id
    or lower(coalesce(primary_profile.email, '')) = lower(coalesce(primary_manager.work_email, ''))
...
'Tenant-scoped processed leave authority history. Delegated actor display identity is resolved from the authenticated actor profile recorded in the immutable audit, with employee identity as a fallback.';
